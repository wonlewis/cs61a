...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        num = dice()
        if num == 1:
            #to keep the dice rolling even if 1 is met
            while num_rolls >1:
                dice()
                num_rolls -=1
            return 1
    ...
    def say(score0, score1):
        if (who==0):
            if ((score0-last_score)>running_high):
                my_highest_score = score0-last_score
                my_score = score0
                print(my_highest_score, "point(s)! That's a record gain for Player 0!")
            elif ((score0-last_score)<=running_high):
                my_highest_score = running_high
                my_score = score0
        
        elif (who==1):
            if ((score1-last_score)>running_high):
                my_highest_score = score1-last_score
                my_score = score1
                print(my_highest_score, "point(s)! That's a record gain for Player 1!")
            elif ((score1-last_score)<=running_high):
                my_highest_score = running_high
                my_score = score1
        return announce_highest(who, my_score, my_highest_score)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def strategy(*args):
        ...
        results = 0
        while counter >0:
            results = results + original_function(*args)
            counter -= 1
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    counter = 1
    highest_no_of_rolls = 0
    highest_average = 0
    averaged_dice = make_averaged(roll_dice, trials_count)
    while (counter<=10):
        roll = averaged_dice(counter, dice)
        if (roll>highest_average):
            highest_no_of_rolls = counter
            highest_average = roll
        counter += 1
    return highest_no_of_rolls
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if (picky_piggy_score+score == opponent_score):
        return 0
    elif (picky_piggy_score>= cutoff):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
