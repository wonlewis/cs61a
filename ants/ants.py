...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        counter = 0
        ans = self.place
        while ans.entrance is not None and counter <= self.max_range:
            if ans.bees != [] and ans.name != 'Hive' and counter >= self.min_range:
                return random_or_none(ans.bees)
            else:
                counter += 1
                ans = ans.entrance
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digest_state == 0:
            if len(self.place.bees)> 0:
                my_bee = random.choice(self.place.bees)
                my_bee.reduce_armor(my_bee.armor)
                self.digest_state = self.time_to_digest
        else:
            self.digest_state -= 1
        # END Problem 6
