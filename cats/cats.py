...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    my_list = [x for x in paragraphs if select(x) == True]
    if (k < len(my_list)):
        return my_list[k]
    else:
        return ""
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def select(x):
        words = split(remove_punctuation(lower(x)))
        for a in words:
            for b in topic:
                if a == b:
                    return True
...
def accuracy(typed, reference):
    ...
    "*** YOUR CODE HERE ***"
    count = 0
    if (len(reference_words)==0 or (len(reference_words)==1 and reference_words[0]=='')) and (len(typed_words)==0 or (len(typed_words)==1 and typed_words[0]=='')):
        return 1/1*100
    if len(typed_words)==0 or (len(typed_words)==1 and typed_words[0]==''):
        return 0/1*100
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for x in range(len(valid_words)):
        if typed_word == valid_words[x]:
            return typed_word
        my_diff = diff_function(typed_word, valid_words[x], limit)
        if my_diff < counter:
            counter = my_diff
            lowest = x
    if counter > limit:
        return typed_word
    return valid_words[lowest]
...
def minimum_mewtations(start, goal, limit):
    ...
    if len(start)==0 and len(goal) > 0:
        ...
        return 1 + min(minimum_mewtations(remove[1:], goal, limit-1), minimum_mewtations(add[1:], goal[1:], limit-1), minimum_mewtations(substitute[1:], goal[1:], limit-1))
        # END
