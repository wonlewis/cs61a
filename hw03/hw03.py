...
def pingpong(n):
    ...
    if n == 1:
        return 1
    return pingpong_math(n) + pingpong(n-1)
...
def missing_digits(n):
    ...
    if n < 100:
        if (n%10 == n//10):
            return 0
        return n%10 - n//10 - 1
    return counter(n) + missing_digits(n//10)

def counter(n):
    if (n%10 == (n//10-n//100*10)):
        return 0
    return n%10 - (n//10-n//100*10) - 1
...
def count_coins(change):
    ...
    def count_partitions(change, largest_coin):
        ...
        if largest_coin == None:
            return 0
        return count_partitions(change - largest_coin, largest_coin) + count_partitions(change, descending_coin(largest_coin))
